"""
=============================================================
  DAILY PREDICTION ENGINE — VERSION 2
  Produced: Czarni Home + ACE Home + MFK Tatran Home
  Combined odds: 2.46 — ALL THREE WON
=============================================================
"""
...
MIN_MODEL_PROB   = 0.55
MAX_MODEL_PROB   = 0.92   # filters out 100% confidence picks
MIN_EV           = 0.02
MIN_ODDS         = 1.20   # user preference
MAX_ODDS         = 4.00
ACCUM_LEGS       = 3
...
print("\n" + "="*65)
print(f"  DAILY PREDICTION ENGINE v2 — {date.today().strftime('%A %d %B %Y')}")
print("="*65)
...
# V2 filters — MAX_MODEL_PROB cap + bookie_odds required
...
accum_lines.append(f"  DAILY {ACCUM_LEGS}-LEG ACCUMULATOR  (v2)")
...
if len(accum_picks) < ACCUM_LEGS:
    accum_lines.append(f"\n  Only {len(accum_picks)} picks met threshold today.")
