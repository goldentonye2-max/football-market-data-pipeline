...
def predict_all_fixtures(fixtures, odds_wide, models, min_edge, min_auc, min_model_prob, max_model_prob):
    """
...
def main():
    ...
    # Sort all best picks by edge descending
    best_picks_df = best_picks_df.sort_values("edge_%", ascending=False).reset_index(drop=True)
    ...
    # ── SAVE TO CSV ─────────────────────────────────────────
    if args.csv:
        best_picks_df.to_csv(OUTPUT_CSV, index=False)
        print(f"\n  ✅ Saved to {OUTPUT_CSV}")

        all_picks_df.to_csv("all_picks_today.csv", index=False)
        print(f"  ✅ All candidate picks saved to all_picks_today.csv")
